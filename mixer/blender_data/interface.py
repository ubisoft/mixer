...
if TYPE_CHECKING:
    ...
    from mixer.blender_data.proxy import Uuid
...
class Interface:
    """Network interface to BpyDataProxy.

    Send updates detected by BpyDataProxy (send_xxx methods) and updates BpyDataProxy from update messages
    (build_xxx methods).
    ...
    def __init__(self, client: Optional[BlenderClient], bpy_data_proxy: BpyDataProxy):
        ...
        self._functions: Dict[MessageType, Callable[[bytes], None]] = {
            MessageType.BLENDER_DATA_UPDATE: self.build_data_update,
            MessageType.BLENDER_DATA_REMOVE: self.build_data_remove,
            MessageType.BLENDER_DATA_CREATE: self.build_data_create,
            MessageType.BLENDER_DATA_RENAME: self.build_data_rename,
            MessageType.BLENDER_DATA_MEDIA: self.build_data_media,
        }

    @property
    def proxy(self) -> BpyDataProxy:
        return self._bpy_data_proxy

    def process(self, command: Command) -> bool:
        try:
            func = self._functions[command.type]
        except KeyError:
            return False
    ...
    def send_media_creations(self, proxy: DatablockProxy):
        ...
        if bytes_ and proxy._media:
            ...
            self._client.add_command(command)

    def send_data_updates(self, updates: UpdateChangeset):
        assert self._client is not None
        for update in updates:
            logger.debug("%s %s", "send_data_update", update)

            try:
                encoded_update = self._codec.encode(update)
            except Exception:
    ...
    def send_data_creations(self, proxies: CreationChangeset):
        assert self._client is not None
        for datablock_proxy in proxies:
            logger.info("%s %s", "send_data_create", datablock_proxy)
            self.send_media_creations(datablock_proxy)
            try:
                encoded_proxy = self._codec.encode(datablock_proxy)
    ...
    def send_data_removals(self, removals: RemovalChangeset):
        assert self._client is not None
    ...
    def send_data_renames(self, renames: RenameChangeset):
        assert self._client is not None
        if not renames:
            return
        items = []
    ...
    def build_data_media(self, buffer: bytes):
    ...
    def build_data_create(self, buffer: bytes):
        ...
        try:
            ...
            logger.error("... proxy serialisation not registered or possible version mismatch")
            return
        except Exception:
        ...
        if self._client and rename_changeset:
            # client is not set when used in server
            self.send_data_renames(rename_changeset)

    def _build_soas(self, uuid: Uuid, soas: List[Soa]):
        update = False
        try:
            for soa in soas:
        ...
        if update:
            ...
            # refactor BpyDataProxy.update_soa() into BpyDataProxy.update_soas() and embed the update there
            try:
    ...
    def build_data_update(self, buffer: bytes):
        codec = Codec()
        try:
            message = BlenderDataMessage()
            message.decode(buffer)
            delta = codec.decode(message.proxy_string)
    ...
    def build_data_remove(self, buffer: bytes):
