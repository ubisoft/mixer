...
from mixer.os_utils import addons_infos, tech_infos
...
def join_room(room_name: str, vrtist_protocol: bool = False, shared_folders=None, ignore_version_check: bool = False):
    ...
    protocol = "VRtist" if vrtist_protocol else "Generic"
    logger.warning(f"join: room: {room_name}, user: {prefs.user}, protocol: {protocol}")

    for line in tech_infos():
        logger.warning(line)

    for line in addons_infos():
        logger.info(line)
