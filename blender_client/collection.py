...
def sendCollection(client: "ClientBlender", collection: bpy.types.Collection):
    ...
    buffer = (
        common.encodeString(collection.name_full)
        + common.encodeBool(not collection.hide_viewport)
        + common.encodeVector3(collectionInstanceOffset)
    )
    client.addCommand(common.Command(common.MessageType.COLLECTION, buffer, 0))
...
def sendAddCollectionToCollection(client: "ClientBlender", parentCollectionName, collectionName):
    ...
    client.addCommand(common.Command(common.MessageType.ADD_COLLECTION_TO_COLLECTION, buffer, 0))
...
def sendRemoveCollectionFromCollection(client: "ClientBlender", parentCollectionName, collectionName):
    ...
    buffer = common.encodeString(parentCollectionName) + common.encodeString(collectionName)
    client.addCommand(common.Command(common.MessageType.REMOVE_COLLECTION_FROM_COLLECTION, buffer, 0))
...
def sendCollectionInstance(client: "ClientBlender", obj):
    ...
    client.addCommand(common.Command(common.MessageType.INSTANCE_COLLECTION, buffer, 0))
